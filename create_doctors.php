...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($existingDoctorResult->num_rows > 0) {
        // Doctor already exists, update the existing record
        $existingDoctorRow = $existingDoctorResult->fetch_assoc();
        $doctorId = $existingDoctorRow['id'];

        $updateSql = "UPDATE doctors SET name = ?, surname = ? WHERE id = ?";
        $updateStmt = $conn->prepare($updateSql);
        $updateStmt->bind_param("ssi", $name, $surname, $doctorId);
